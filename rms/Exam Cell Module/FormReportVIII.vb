...
Public Class FormReportVIII
    ...
    Private Sub FormReportVIII_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        FormStyles(CallingForm:=Me, Text:="RMS | Collegewise Merit List")   ' Form Styles
...
collegewise_courses.UNIVERSITYWISE_COURSE_ID=universitywise_courses.UNIVERSITYWISE_COURSE_ID AND collegewise_courses.COLLEGE_ID='{CollegeID}';"
    ...
    Private Sub ComboBoxSession_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ComboBoxSession.SelectedIndexChanged
        If CountCollege <> 0 And CountCourse <> 0 And CountSemester <> 0 And ComboBoxCollege.SelectedItem IsNot Nothing And ComboBoxCourse.SelectedItem IsNot Nothing And
              ComboBoxSemester.SelectedItem IsNot Nothing Then
            ButtonGenerateReport.Enabled = True
        Else
            ButtonGenerateReport.Enabled = False
    ...
    Private Sub ComboBoxSemester_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ComboBoxSemester.SelectedIndexChanged
        If CountCollege <> 0 And CountCourse <> 0 And CountSession <> 0 And ComboBoxCollege.SelectedItem IsNot Nothing And ComboBoxCourse.SelectedItem IsNot Nothing And
             ComboBoxSession.SelectedItem IsNot Nothing Then
            ButtonGenerateReport.Enabled = True
        Else
            ButtonGenerateReport.Enabled = False
        End If
    ...
    Private Sub ComboBoxCourse_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ComboBoxCourse.SelectedIndexChanged
        If CountCollege <> 0 And CountSession <> 0 And ComboBoxCollege.SelectedItem IsNot Nothing And
             ComboBoxSession.SelectedItem IsNot Nothing And CountSemester <> 0 And ComboBoxSemester.SelectedItem IsNot Nothing Then
            ButtonGenerateReport.Enabled = True
        Else
            ButtonGenerateReport.Enabled = False
        End If
    ...
    Private Sub ShowReport()
        ...
        Try
            ...
            cmd.CommandText = $"SELECT SUM((EXTERNAL_MARKS+INTERNAL_MARKS)), marksheets.UNIVERSITY_ROLL_NUMBER, CONCAT(FIRST_NAME, ' ', LAST_NAME), COLLEGE_NAME
    FROM students, colleges, universities, subjects, marksheets, semesters, sessionwise_semesters, result_criteria, courses WHERE colleges.COLLEGE_ID=students.COLLEGE_ID
    AND subjects.SUBJECT_ID=marksheets.SUBJECT_ID AND marksheets.SESSIONWISE_SEMESTER_ID=(SELECT SESSIONWISE_SEMESTER_ID FROM sessionwise_semesters WHERE SESSION_ID=
    '{ComboBoxSession.SelectedItem}' AND SEMESTER_ID=(SELECT SEMESTER_ID FROM semesters WHERE SEMESTER='{ComboBoxSemester.SelectedItem}')) AND
    universities.UNIVERSITY_ID=colleges.UNIVERSITY_ID AND students.UNIVERSITY_ROLL_NUMBER=marksheets.UNIVERSITY_ROLL_NUMBER AND
    ...
    result_criteria.COURSE_ID=marksheets.COURSE_ID AND courses.COURSE_ID=marksheets.COURSE_ID AND colleges.COLLEGE_ID=(SELECT COLLEGE_ID FROM
    colleges WHERE COLLEGE_NAME='{ComboBoxCollege.SelectedItem}') AND marksheets.COURSE_ID=(SELECT COURSE_ID FROM courses WHERE 
