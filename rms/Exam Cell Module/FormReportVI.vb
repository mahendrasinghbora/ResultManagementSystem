...
Public Class FormReportVI
    ...
    Private Sub FormReportVI_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        FormStyles(CallingForm:=Me, Text:="RMS | Universitywise Merit List")   ' Form Styles
...
universitywise_courses.COURSE_ID=courses.COURSE_ID AND universitywise_courses.UNIVERSITY_ID='{UniversityID}';"
    ...
    Private Sub ComboBoxCourse_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ComboBoxCourse.SelectedIndexChanged
        If CountUniversity <> 0 And CountSession <> 0 And ComboBoxUniversity.SelectedItem IsNot Nothing And
    ...
    COURSE_NAME='{ComboBoxCourse.SelectedItem}') AND marksheets.STATUS='1' GROUP BY marksheets.UNIVERSITY_ROLL_NUMBER ORDER BY SUM((EXTERNAL_MARKS+INTERNAL_MARKS)) DESC;"
            ...
            'myReport.Load("C:\Users\Mahendra Singh Bora\Documents\VBprojects\rms\rms\Reports\CrystalReport6.rpt")
            myReport.Load(Application.StartupPath.Replace("\bin\x64\Debug", "\Reports\CrystalReport6.rpt"))
    ...
    COURSE_NAME='{ComboBoxCourse.SelectedItem}') AND marksheets.STATUS='1' GROUP BY marksheets.UNIVERSITY_ROLL_NUMBER ORDER BY SUM((EXTERNAL_MARKS+INTERNAL_MARKS)) DESC;"
            ...
            'myData.WriteXml("C:\Users\Mahendra Singh Bora\Documents\VBprojects\rms\rms\Reports\dataset6.xml", XmlWriteMode.WriteSchema)
            myData.WriteXml(Application.StartupPath.Replace("\bin\x64\Debug", "\Reports\dataset6.xml"), XmlWriteMode.WriteSchema)
        Catch ex As Exception
            MessageBox.Show(text:=ex.Message, caption:="Report could not be created.", buttons:=MessageBoxButtons.OK, icon:=MessageBoxIcon.Error)
        End Try
    End Sub
